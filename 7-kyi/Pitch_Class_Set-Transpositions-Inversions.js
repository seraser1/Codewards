...
function operate(pcSet, operation) {
    ...
    const n = +operation.match(/\d+/).join('');
    const res  = pcSet.map(el => (el + n) % 12).sort((a,b) => a - b);
    const resI = pcSet.map(el => (12 - el + n) % 12).sort((a,b) => a - b);
    return !isInversion ? res : resI
}
console.log(operate([4,6,7,10], "T1"));
